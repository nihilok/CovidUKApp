...
class TableData(RecycleView):
    ...
    def set_text_from_of(self, direction):
        ...
        if self.pagination:
            if direction == "forward" and self._row_data_parts:
                if (
    ...
    def set_next_row_data_parts(self, direction):
        ...
        if self._to_value == len(self.row_data) or len(self.row_data) == 0:
            self.pagination.ids.button_forward.disabled = True
        if self._current_value == 1 or len(self.row_data) == 0:
...
class MDDataTable(BaseDialog):
    ...
    def __init__(self, **kwargs):
        ...
        Clock.schedule_once(self.create_pagination_menu)
